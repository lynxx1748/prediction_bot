...
class ModelEvaluator:
    ...
    def __init__(self, model_name, metrics_file=None):
        ...
        self.metrics_file = (
            metrics_file
            or Path(config.get("paths.metrics_dir", "metrics"))
    ...
    def increment_version(self, reason=None):
        ...
        logger.info(
            f"Model {self.model_name} version incremented to {self.current_version}: {reason}"
        )
    ...
    def record_metrics(self, metrics_dict, prediction_count=None):
        ...
        if success:
            logger.info(
                f"Recorded metrics for {self.model_name} v{self.current_version}: {metrics_dict}"
            )
    ...
    def get_performance_trend(self, metric_name, last_n=10):
        ...
        direction = (
            "improving" if slope > 0.01 else "declining" if slope < -0.01 else "stable"
        )
    ...
    def should_retrain(self):
        ...
        if (
            "current" in accuracy_trend
            and accuracy_trend["current"] is not None
            and accuracy_trend["current"] < accuracy_threshold
        ):
            return (
                True,
                f"Accuracy below threshold: {accuracy_trend['current']:.2f} < {accuracy_threshold:.2f}",
            )

        # Check for declining accuracy trend
        if (
            "current" in accuracy_trend
            and "previous" in accuracy_trend
            and accuracy_trend["current"] is not None
            and accuracy_trend["previous"] is not None
            and accuracy_trend["current"] < accuracy_trend["previous"] * 0.9
        ):
            return (
                True,
                f"Declining accuracy: {accuracy_trend['current']:.2f} < {accuracy_trend['previous']:.2f}",
            )
        ...
        if days_since_training > max_days_without_training:
            return (
                True,
                f"Time since last training: {days_since_training} days > {max_days_without_training} days",
            )
        ...
        if self.samples_since_training > 200:
            return (
                True,
                f"Many new samples since training: {self.samples_since_training} > 200",
            )
    ...
    def plot_metrics(self, metric_names=None, save_path=None):
        ...
        try:
            ...
            if metric_names is None:
                ...
                metric_names = [
                    col for col in metrics_df.columns if col not in ["date", "version"]
                ]
            ...
            for metric in metric_names:
                if metric in metrics_df.columns:
                    plt.plot(
                        metrics_df["date"], metrics_df[metric], marker="o", label=metric
                    )
            ...
            for version_info in self.metrics_history["versions"]:
                ...
                if (
                    version_date >= metrics_df["date"].min()
                    and version_date <= metrics_df["date"].max()
                ):
    ...
    def _get_days_since_training(self):
        ...
        try:
            ...
            latest_version = max(
                self.metrics_history["versions"], key=lambda v: v["version"]
            )
