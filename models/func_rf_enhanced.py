...
class AdaptiveRandomForest:
    ...
    def __init__(self, model_name="adaptive_rf"):
        ...
        self.recency_halflife = self.config.get(
            "recency_halflife", 500
        )  # Samples after which weight is halved
        ...
        # Setup file paths
        self.model_path = (
            Path(config.get("paths.model_file", "models")) / f"{model_name}.joblib"
        )
    ...
    def load_historical_data(self, db_file=None, table=None, limit=10000):
        ...
        try:
            ...
            # Calculate outcome
            df["outcome"] = df.apply(
                lambda row: "BULL" if row["closePrice"] > row["lockPrice"] else "BEAR",
                axis=1,
            )
    ...
    def train(self, force=False, save=True):
        ...
        try:
            ...
            if total_samples < self.min_samples and not force:
                logger.warning(
                    f"Not enough samples for training: {total_samples} < {self.min_samples}"
                )
                return False

            # Combine historical data with recent predictions
            all_data = self.training_data + [
                p for p in self.recent_predictions if p.get("actual") is not None
            ]
            ...
            if self.recency_weight and len(df) > 10:
                ...
                sample_weights = np.exp(
                    -np.arange(len(df))[::-1] / self.recency_halflife
                )
                # Normalize weights
            ...
            # Log results
            logger.info(
                f"Training completed in {time.time() - start_time:.2f}s with {training_accuracy:.2%} accuracy"
            )
        ...
        try:
            ...
            for pred in self.recent_predictions:
                if pred["epoch"] == epoch and pred["actual"] is None:
                    ...
                    logger.info(
                        f"Prediction for epoch {epoch} was {'correct' if correct else 'incorrect'} (predicted {pred['prediction']}, actual {actual_outcome})"
                    )
            ...
            completed_predictions = sum(
                1 for p in self.recent_predictions if p.get("actual") is not None
            )
            if completed_predictions >= self.config.get("retrain_threshold", 20):
                logger.info(
                    f"Accumulated {completed_predictions} completed predictions, considering retraining"
                )
