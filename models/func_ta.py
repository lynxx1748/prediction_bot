...
class TechnicalAnalysis:
    ...
    def detect_candlestick_patterns(opens, highs, lows, closes):
        ...
        patterns["bullish_engulfing"] = (
            ...
            and (closes[-1] - opens[-1] > opens[-2] - closes[-2])
        )
        ...
        patterns["bearish_engulfing"] = (
            (opens[-1] > closes[-2])
            and (closes[-1] < opens[-2])
            and (opens[-1] - closes[-1] > closes[-2] - opens[-2])
        )

        # Check for hammer (bullish)
        patterns["hammer"] = (
            (closes[-1] > opens[-1])
            and ((highs[-1] - closes[-1]) < 0.2 * (closes[-1] - lows[-1]))
            and ((closes[-1] - opens[-1]) < 0.3 * (closes[-1] - lows[-1]))
        )

        # Check for shooting star (bearish)
        patterns["shooting_star"] = (
            (closes[-1] < opens[-1])
            and ((closes[-1] - lows[-1]) < 0.2 * (highs[-1] - closes[-1]))
            and ((opens[-1] - closes[-1]) < 0.3 * (highs[-1] - closes[-1]))
        )
    ...
    def get_candlestick_patterns():
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"❌ Error fetching candlestick data: HTTP {response.status_code}"
                )
                return None, 0
            ...
            # Detect patterns
            patterns = TechnicalAnalysis.detect_candlestick_patterns(
            ...
            bearish_signals = sum(
                [patterns["bearish_engulfing"], patterns["shooting_star"]]
            )
            ...
            if pattern_signal:
                ...
                logger.info(
                    f"   Signal: {pattern_signal.upper()} (strength: {strength:.2f})"
                )
        ...
        try:
            ...
            if active_round_data[4] > 0:  # lockPrice is set
                ...
                if current_price and active_round_data[4] > 0:
                    ...
                    lockPrice = web3.from_wei(
                        active_round_data[4], "ether"
                    )  # Convert from wei
                    ...
                    if lockPrice_float < 0.001 or lockPrice_float > 10000:
                        logger.warning(
                            f"⚠️ Unrealistic lock price detected: {lockPrice_float}, using current price"
                        )
                        lockPrice_float = current_price_float

                    # Calculate percentage change
                    price_change_percent = (
                        (current_price_float - lockPrice_float) / lockPrice_float
                    ) * 100

                    # Cap unrealistic price changes
                    if abs(price_change_percent) > 20:
                        logger.warning(
                            f"⚠️ Unrealistic price change detected: {price_change_percent:.2f}%, capping at ±5%"
                        )
                        price_change_percent = 5.0 if price_change_percent > 0 else -5.0

                    logger.info(
                        f"📊 Active round price change: {price_change_percent:.2f}%"
                    )

                    # Check for significant price movements
                    if price_change_percent < -4.0:
                        logger.info(
                            f"📉 MAJOR drop detected in active round: {price_change_percent:.2f}%"
                        )
                        return "BULL", min(0.95, abs(price_change_percent) / 8)
                    elif price_change_percent < -2.0:
                        logger.info(
                            f"📉 Significant drop detected in active round: {price_change_percent:.2f}%"
                        )
                        return "BULL", min(0.85, abs(price_change_percent) / 10)
                    elif price_change_percent > 4.0:
                        logger.info(
                            f"📈 MAJOR rise detected in active round: {price_change_percent:.2f}%"
                        )
                        return "BEAR", min(0.85, price_change_percent / 8)
                    elif price_change_percent > 2.0:
                        logger.info(
                            f"📈 Significant rise detected in active round: {price_change_percent:.2f}%"
                        )
            ...
            if price_changes:
                if price_changes[0] < -3.0:
                    ...
                    logger.info(
                        f"📉 Detected potential rebound after {price_changes[0]:.2f}% drop in previous round"
                    )
                    return "BULL", strength

                elif consecutive_same_direction >= 2:
                    avg_change = sum(
                        price_changes[: consecutive_same_direction + 1]
                    ) / (consecutive_same_direction + 1)
                    strength = min(0.8, abs(avg_change) / 5)
                    prediction = "BULL" if last_direction == "up" else "BEAR"
                    logger.info(
                        f"📈 Detected trend continuation: {consecutive_same_direction + 1} rounds of {last_direction}"
                    )
    ...
    def calculate_indicators(self, prices):
        ...
        try:
            ...
            indicators = {
                ...
                if isinstance(middle_band, (int, float))
                else middle_band[-1]
...
def get_technical_indicators(prices, config_override=None):
    ...
    try:
        ...
        if use_bollinger:
            ...
            logger.info(
                f"📊 Bollinger Bands: Position {bb_position:.2f} (0=lower, 1=upper)"
            )
        ...
        if use_candlestick and len(prices) >= 5:
            ...
            try:
                ...
                if pattern_signal:
                    ...
                    logger.info(
                        f"🕯️ Candlestick Pattern: {pattern_signal} (strength: {pattern_strength:.2f})"
                    )
            except Exception as e:
        ...
        if bull_signals and not bear_signals:
            ...
            logger.info(
                f"📊 Signal Strengths - Bull: {bull_strength:.2f}, Bear: {bear_strength:.2f}"
            )
