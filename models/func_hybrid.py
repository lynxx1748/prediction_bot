    ...
    try:
        ...
        predictions = {
            "pattern": (
                pattern_prediction,
                pattern_confidence,
                cfg.get("STRATEGY_WEIGHTS", {}).get("pattern", 0.20),
            ),
            "market": (
                market_prediction,
                market_confidence,
                cfg.get("STRATEGY_WEIGHTS", {}).get("market", 0.20),
            ),
        }
        ...
        logger.info(
            f"✅ Hybrid prediction: {final_prediction} (confidence: {confidence:.2f})"
        )
...
def hybrid_prediction_old(round_data, model=None, scaler=None, contract=None):
    ...
    try:
        ...
        logger.info(
            f"Using features: bullRatio={round_data['bullRatio']:.2f}, bearRatio={round_data['bearRatio']:.2f}, "
        ...
        final_prediction, final_confidence = ensemble_prediction(
            round_data, market_data, config
        )
        ...
        if contrarian_mode:
            ...
            final_confidence = (
                0.7  # Set a reasonable confidence for contrarian predictions
            )
...
def hybrid_prediction_with_learning(
    round_data, technical_data, market_data, prediction_history=None
):
    """
    ...
    if len(prediction_history["predictions"]) > 10:
        ...
        for component in ["pattern", "market", "technical", "sentiment"]:
            if component in prediction_history["performance"]:
                component_accuracy[component] = prediction_history["performance"][
                    component
                ]
    ...
    for pred in prediction_history["predictions"]:
        if pred["epoch"] == epoch and pred["actual"] is None:
            ...
            for component, prediction in pred.get("components", {}).items():
                ...
                prediction_history["performance"][component] = (
                    0.75 * component_correct
                ) + (0.25 * current)
