...
class AIStrategy:
    ...
    def __init__(self, config_override=None):
        ...
        self.retrain_frequency = self.config.get('training', {}).get('retrain_frequency', 10)
        self.samples_since_training = 0
        self.feature_weights = self.config.get('training', {}).get('feature_weights', {})
        ...
        if self.config.get('models', {}).get('random_forest', {}).get('enabled', True):
            ...
            self.model_params = self.config.get('models', {}).get('random_forest', {}).get('parameters', {
                'n_estimators': 100,
                'max_depth': 5,
            ...
            self.model_params = self.config.get('models', {}).get('xgboost', {}).get('parameters', {
                'n_estimators': 100,
                'max_depth': 6,
                'learning_rate': 0.1
            })
    ...
    def _initialize_model(self):
        ...
        try:
            if os.path.exists(self.model_path):
                ...
                logger.info(f"Loaded AI model (version {self.evaluator.current_version}) from {self.model_path}")
                return True
            else:
                logger.info("No existing AI model found, will train when sufficient data is available")
                return False
    ...
    def train(self, force=False, limit=1000):
        ...
        try:
            ...
            if not force and self.rounds_since_training < self.min_rounds_between_training:
                logger.info(f"Not enough new rounds ({self.rounds_since_training}) since last training")
                return False
            ...
            if force or training_accuracy > 0.60:
                reason = f"Forced training" if force else f"Improved accuracy to {training_accuracy:.2f}"
            ...
            joblib.dump((self.model, self.scaler, available_features, new_version), self.model_path)
            ...
            logger.info(f"AI model trained successfully (v{new_version}). "
                       f"Accuracy: {training_accuracy:.2f}, Samples: {len(df)}")
            
            # Calculate and log class distribution
            class_distribution = df['outcome_binary'].value_counts(normalize=True).to_dict()
    ...
    def predict(self, data):
        ...
        try:
            ...
            try:
                ...
                confidence = min(0.5 + confidence, 0.95)  # Scale confidence between 0.5-0.95
                
                # Log both probabilities for analysis
                logger.info(f"AI Model Probabilities: BEAR={bear_prob:.4f}, BULL={bull_prob:.4f}")
                logger.info(f"AI Prediction: {prediction} with confidence {confidence:.4f}")
    ...
    def _check_retrain_condition(self):
        ...
        try:
            ...
            if time_since_training > self.training_interval:
                logger.info(f"Retraining model due to time interval ({time_since_training/3600:.1f} hours since last training)")
            ...
            if self.rounds_since_training >= self.min_rounds_between_training:
                logger.info(f"Retraining model due to accumulated rounds ({self.rounds_since_training})")
                self.train()
                return
    ...
    def learn_from_failures(self):
        ...
        try:
            ...
            if 'error' in analysis:
                logger.warning(f"Could not analyze failed predictions: {analysis['error']}")
                return False
            ...
            for feature, stats in feature_stats.items():
                ...
                if stats['std'] < 0.1 and (stats['mean'] > 0.8 or stats['mean'] < 0.2):
                    ...
                    new_weight = max(0.1, min(3.0, 1.0 / self.feature_weights.get(feature, 1.0)))
                    adjusted_weights[feature] = new_weight
                    pattern_detected = True
                    logger.info(f"Adjusting weight for {feature} from {self.feature_weights.get(feature, 1.0):.2f} to {new_weight:.2f}")
            ...
            if pattern_detected:
                ...
                self.evaluator.increment_version(f"Adjusted weights based on {analysis['count']} failed predictions")
                return True
            
            # If no clear pattern, but have many failures, consider more drastic action
            if analysis['count'] > 20:
                logger.warning("Many failures with no clear pattern - considering model rebuild")
                if analysis.get('confidence_stats', {}).get('mean', 0) > 0.7:
                    # High confidence but wrong - try inverting prediction strategy
                    logger.warning("High confidence failures detected - inverting prediction approach")
                    # Invert all feature weights
                    for feature in self.feature_weights:
                        self.feature_weights[feature] = 1.0 / max(0.1, self.feature_weights[feature])
                
                    self.evaluator.increment_version("Inverted weights due to high-confidence failures")
                    return True
            ...
            for i in range(iterations):
                ...
                if initial_performance and new_performance:
                    ...
                    if new_acc > old_acc + 0.05:  # 5% improvement
                        logger.info(f"Optimization successful: Accuracy improved from {old_acc:.2f} to {new_acc:.2f}")
                        results['ending_accuracy'] = new_acc
                        # Increment version to mark the improvement
                        self.evaluator.increment_version(f"Self-optimization improved accuracy by {new_acc-old_acc:.2f}")
    ...
    def load_model(self):
        ...
        try:
            import pickle
            from sklearn.ensemble import RandomForestClassifier
            from scripts.core.constants import MODEL_FILE, SCALER_FILE
            
            # Check if model files exist
            if not os.path.exists(MODEL_FILE) or not os.path.exists(SCALER_FILE):
                logger.warning(f"❌ Model files not found: {MODEL_FILE} or {SCALER_FILE}")
                # Initialize new model instead of failing
                self.model = RandomForestClassifier(**self.model_params)
                self.scaler = StandardScaler()
                logger.info(f"✅ Initialized new AI model (no previous model found)")
                return True
            ...
            try:
                ...
                logger.error(f"❌ Error loading model files (corrupted?): {load_error}")
                # Initialize new model instead of failing
                self.model = RandomForestClassifier(**self.model_params)
                self.scaler = StandardScaler()
                logger.info(f"✅ Initialized new AI model due to load error")
                return True
