...
from scripts.trading.betting import place_bet, should_place_bet, check_for_win
    ...
    while True:
        ...
        try:
            ...
            if choice == "1":
                ...
                confirm = input("⚠️ WARNING: Live mode will place REAL bets with REAL money. Are you sure? (yes/no): ").strip().lower()
                if confirm == "yes":
                    return "live"
                else:
...
def is_betting_time(seconds_until_lock):
    ...
    min_seconds = config.get('timing', {}).get('optimal_betting_seconds_before_lock', {}).get('min', 30)
    max_seconds = config.get('timing', {}).get('optimal_betting_seconds_before_lock', {}).get('max', 180)
...
def display_round_info(epoch):
    ...
    try:
        ...
        if round_data:
            print(f"\n📊 Round {epoch} Data:")
            print(f"   Bull/Bear Ratio: {round_data.get('bullRatio', 0):.2f}/{round_data.get('bearRatio', 0):.2f}")
...
def display_recent_trades(limit=5):
    ...
    try:
        ...
        for trade in trades:
            ...
            print(f"   {result_emoji} Epoch {epoch}: {prediction} vs {outcome} → {profit_sign}{profit_loss:.6f} BNB")
...
def analyze_market_context(round_data):
    ...
    """
    try:
        from scripts.analysis.market import get_market_sentiment, get_market_direction
        from scripts.analysis.technical import get_technical_prediction
...
def get_high_confidence_predictions(all_predictions, min_confidence=0.75):
    ...
    try:
        ...
        market_regime = detect_market_regime(get_historical_prices(20)).get('regime', 'unknown')
        
        # Pass the market_regime parameter to filter_signals
        filtered_predictions = filter_signals(all_predictions, min_confidence, market_regime)
...
def select_best_strategy(market_regime, round_data):
    ...
    try:
        ...
        print(f"🧠 Selected strategy: {strategy.get('primary')} (score: {strategy.get('score', 0):.2f})")
...
def initialize_ai_strategy():
    ...
    try:
        ...
        if loaded:
            print(f"✅ AI model loaded successfully (version {ai_strategy.model_version})")
        else:
        ...
        if force or hours_since_optimization >= 24:  # Optimize daily or when forced
            ...
            if results.get('retrained'):
                ...
                if before_performance and after_performance:
                    ...
                    print(f"📈 Accuracy changed by {change:.2%} ({before_acc:.2%} → {after_acc:.2%})")
            else:
                print("ℹ️ AI self-optimization completed with no significant improvements")
