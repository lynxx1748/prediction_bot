...
try:
    try:
        try:
            ...
            subprocess.check_call(["python3", "-m", "pip", "install", "keyboard"])
            import keyboard  # type: ignore[import]
    except ImportError:
        keyboard = None
...
def optimize_swing_trading(recent_performance, market_stats, consecutive_losses=0):
    ...
    try:
        ...
        if not is_valid or sample_size < 10:
            logger.info(
                "📊 Not enough data for swing trade optimization, using default parameters"
            )
            return params
        ...
        if abs(bull_win_rate - bear_win_rate) > 0.2:
            ...
            if bull_win_rate > bear_win_rate:
                logger.info(
                    f"📊 BULL predictions more accurate ({bull_win_rate:.2f} vs {bear_win_rate:.2f})"
                )
                params["skip_if_trend_against"] = (
                    bull_win_rate < 0.5
                )  # Skip if BULL accuracy below 50%
            else:
                logger.info(
                    f"📊 BEAR predictions more accurate ({bear_win_rate:.2f} vs {bull_win_rate:.2f})"
                )
                params["skip_if_trend_against"] = (
                    bear_win_rate < 0.5
                )  # Skip if BEAR accuracy below 50%
        ...
        logger.info(
            f"📊 Swing trading parameters optimized: threshold={params['confidence_threshold']}, risk={params['risk_factor']}"
        )
        return params
...
def detect_swing_pattern(price_changes, market_stats=None):
    ...
    try:
        ...
        result = {"swing_opportunity": False, "direction": None, "confidence": 0.0}

        # If not enough data, return default result
        if (
            not price_changes
            or not isinstance(price_changes, list)
            or len(price_changes) < 3
        ):
            return result
