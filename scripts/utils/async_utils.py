...
async def timeout_resistant_prediction(
    prediction_coroutine, timeout=2.0, default_result=("BULL", 0.51)
):
    """
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Async prediction timed out after {timeout} seconds - using fallback"
            )
...
def run_prediction_with_timeout(
    async_function, data, timeout=2.0, default_result=("BULL", 0.51)
):
    """
    ...
    try:
        ...
        if loop.is_running():
            ...
            future = asyncio.run_coroutine_threadsafe(
                timeout_resistant_prediction(coroutine, timeout), loop
            )
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ Thread-safe async execution timed out after {timeout + 0.5} seconds"
                )
                return default_result
        else:
            # Normal case - run the coroutine with timeout handling
            return loop.run_until_complete(
                timeout_resistant_prediction(coroutine, timeout)
            )
