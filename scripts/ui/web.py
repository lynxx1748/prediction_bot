            ...
            with open(log_file, "r") as f:
                ...
                if logs:
                    ...
                    def calculate_period_stats(start_date):
                        ...
                        total = len(
                            [log for log in period_logs if log.get("won") is not None]
                        )
                        profit = sum(
                            float(log.get("profit_loss", 0)) for log in period_logs
                        )
                        win_rate = (wins / total * 100) if total > 0 else 0
                        return profit, win_rate

                    today_profit, today_winrate = calculate_period_stats(
...
def start_web_interface():
    ...
    try:
        ...
        web_thread.daemon = (
            True  # This ensures the thread stops when the main program stops
        )
        web_thread.start()
