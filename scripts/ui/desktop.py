...
class UglyBotControlPanel:
    ...
    def create_status_display(self):
        ...
        try:
            ...
            self.status_wallet = ttk.Label(frame, text="Wallet: Not Connected", style="Holo.TLabel")
            self.status_wallet.pack(anchor='w', padx=10, pady=2)
            
            self.status_network = ttk.Label(frame, text="Network: Unknown", style="Holo.TLabel")
        ...
        try:
            ...
            try:
                ...
                if trades:
                    ...
                    self.lbl_profit_loss.config(text=f"Profit/Loss: {profit_loss:.6f} BNB")
            except Exception as e:
    ...
    def start_bot(self):
        ...
        self.log_text.insert(tk.END, f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Bot started\n")
    ...
    def stop_bot(self):
        ...
        self.log_text.insert(tk.END, f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Bot stopped\n")
        self.log_text.see(tk.END)
    ...
    def open_settings(self):
        ...
        self.log_text.insert(tk.END, f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Opening settings\n")
        self.log_text.see(tk.END)
