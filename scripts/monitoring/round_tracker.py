...
def initialize_mid_round_tracking(epoch, initial_prediction):
    ...
    try:
        ...
        if not hasattr(initialize_mid_round_tracking, 'monitor_thread') or not initialize_mid_round_tracking.monitor_thread.is_alive():
            initialize_mid_round_tracking.monitor_thread = threading.Thread(target=monitor_mid_round_prices, daemon=True)
...
def detect_mid_round_swing():
    ...
    try:
        ...
        recent_movement = recent_changes[-3:] if len(recent_changes) >= 3 else recent_changes
        ...
        if current_round_data['initial_prediction'] == "BULL" and cumulative_change < -0.01:
            ...
            if not current_round_data['swing_detected']:
                ...
                logger.warning(f"⚠️ MID-ROUND SWING DETECTED: {current_round_data['initial_prediction']} → BEAR")
                logger.warning(f"   Time: {timestamps[-1].strftime('%H:%M:%S')}, Movement: {cumulative_change:.4f}%")
                
                # Record the swing in database
                record_mid_round_swing(current_round_data['epoch'], 
                                     current_round_data['initial_prediction'],
                                     "BEAR", 
                                     cumulative_change)
                swing_detected = True
                
        elif current_round_data['initial_prediction'] == "BEAR" and cumulative_change > 0.01:
            ...
            if not current_round_data['swing_detected']:
                ...
                logger.warning(f"⚠️ MID-ROUND SWING DETECTED: {current_round_data['initial_prediction']} → BULL")
                logger.warning(f"   Time: {timestamps[-1].strftime('%H:%M:%S')}, Movement: {cumulative_change:.4f}%")
...
def get_mid_round_swing_statistics(lookback=20):
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='mid_round_swings'")
        ...
        # Get overall count
        cursor.execute("SELECT COUNT(*) FROM mid_round_swings ORDER BY epoch DESC LIMIT ?", (lookback,))
...
def calculate_swing_confidence():
    ...
    try:
        ...
        if len(recent_prices) >= 2:
            ...
            changes = [abs(recent_prices[i]/recent_prices[i-1] - 1) for i in range(1, len(recent_prices))]
...
def analyze_round_progress(current_price, lock_price, elapsed_seconds):
    ...
    try:
        ...
        return {
            ...
            'time_elapsed_pct': (elapsed_seconds / 360) * 100  # Percentage of round complete
        }
