...
from scripts.data.blockchain import get_round_data, get_current_epoch, get_enriched_round_data
from scripts.data.database import record_trade, get_recent_rounds, initialize_prediction_db
...
class DataFlowMonitor:
    ...
    def start(self):
        ...
        self.thread.start()
        logger.info(f"📊 Data flow monitoring started (checking every {self.interval} seconds)")
        return True
    ...
    def _check_data_flow(self):
        ...
        if blockchain_ok and database_ok:
            ...
            logger.info(f"✅ Data flow healthy - Stored {self.stats['rounds_stored']}/{self.stats['rounds_checked']} rounds")
        else:
            self.stats['last_status'] = 'ISSUES DETECTED'
            logger.warning(f"⚠️ Data flow issues - BC: {'✅' if blockchain_ok else '❌'}, DB: {'✅' if database_ok else '❌'}")
    ...
    def _check_database(self):
        ...
        try:
            ...
            cursor.execute(f"SELECT name FROM sqlite_master WHERE type='table' AND name='{TABLES['trades']}'")
    ...
    def _check_data_storage(self):
        ...
        try:
            ...
            cursor.execute(f"SELECT * FROM {TABLES['trades']} WHERE epoch = ?", (test_epoch,))
            ...
            if data:
                ...
                logger.error(f"❌ Could not retrieve data for epoch {test_epoch} from database")
                self.stats['database_errors'] += 1
                return False
    ...
    def _collect_missing_data(self, current_epoch, limit=25):
        ...
        for epoch in range(current_epoch - limit, current_epoch):
            ...
            cursor.execute(f"SELECT epoch FROM {TABLES['trades']} WHERE epoch = ?", (epoch,))
            if cursor.fetchone():
                continue
            ...
            try:
                ...
                if round_data:
                    if record_trade(epoch, round_data):
                        ...
                        logger.error(f"❌ Failed to record missing data for epoch {epoch}")
            except Exception as e:
    ...
    def get_status(self):
        ...
        return {
            ...
            'stats': self.stats,
            'health': 'Good' if self.stats['last_status'] == 'HEALTHY' else 'Issues Detected'
        }
