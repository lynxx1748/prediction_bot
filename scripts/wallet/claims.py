...
from ..core.constants import (ACCOUNT_ADDRESS, PRIVATE_KEY, config, contract,
                              web3)
...
def claim_rewards(epoch, gas_strategy="medium"):
    ...
    try:
        ...
        if gas_price < network_min_gas:
            ...
            logger.warning(
                f"Increased gas price to minimum network requirement: {web3.from_wei(gas_price, 'gwei')} gwei"
            )
        ...
        if balance < estimated_cost:
            ...
            logger.warning(
                f"Required: {web3.from_wei(estimated_cost, 'ether'):.6f} BNB"
            )
            return False
        ...
        logger.info(
            f"Using gas strategy: {gas_strategy}, Gas price: {web3.from_wei(gas_price, 'gwei'):.2f} gwei, Gas limit: {gas_limit}"
        )
        ...
        logger.info(
            f"Waiting for claim transaction to be mined... (tx: {tx_hash.hex()})"
        )
        ...
        if "transaction underpriced" in str(e):
            ...
            if min_gas_match:
                ...
                logger.warning(
                    f"Transaction underpriced. Retrying with minimum gas: {web3.from_wei(min_gas_needed, 'gwei')} gwei"
                )

                # Retry with the exact minimum gas needed plus a small buffer
                return claim_rewards(
                    epoch, "medium"
                )  # Use medium strategy which should be higher
        ...
        for round_epoch in list(placed_bets.keys()):
            ...
            if actual_outcome:
                ...
                if our_prediction.upper() == actual_outcome.upper():
                    ...
                    consecutive_losses = 0
                    logger.info(
                        f"Round {round_epoch}: {our_prediction.upper()} bet WON!"
                    )
                    ...
                    logger.info(
                        f"Round {round_epoch}: {our_prediction.upper()} bet LOST!"
                    )
