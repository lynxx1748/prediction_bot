...
def calculate_bet_amount(config, confidence=0.5, consecutive_losses=0):
    ...
    try:
        ...
        base_amount = float(
            trading_config.get("wager_amount", 0.005)
        )  # Default 0.005 BNB
...
def place_bet(epoch, prediction, amount, gas_strategy="medium"):
    ...
    try:
        ...
        if balance < estimated_cost:
            logger.warning(
                f"Insufficient funds to place {prediction} bet for epoch {epoch}"
            )
        ...
        logger.info(
            f"Gas price: {web3.from_wei(gas_price, 'gwei'):.2f} gwei, Gas limit: {gas_limit}"
        )
        ...
        if tx_receipt.status == 1:
            logger.info(
                f"Successfully placed {prediction} bet of {amount} BNB for epoch {epoch}"
            )
            return True
        else:
