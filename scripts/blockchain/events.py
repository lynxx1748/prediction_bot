...
from ..core.constants import CONTRACT_ADDRESS, DB_FILE, contract, web3
...
def track_betting_events(bull_filter=None, bear_filter=None):
    ...
    try:
        ...
        logger.info(
            f"📊 Round {current_epoch} Data: Bull {bull_ratio:.2%} / Bear {bear_ratio:.2%}"
        )
        return 1
...
def check_new_events(bull_filter, bear_filter):
    ...
    try:
        ...
        for entry in bull_entries:
            ...
            logger.info(
                f"🐂 BetBull: {amount:.4f} BNB from {sender[:8]}... on epoch {epoch}"
            )
        ...
        for entry in bear_entries:
            ...
            sender = entry["args"]["sender"]
            logger.info(
                f"🐻 BetBear: {amount:.4f} BNB from {sender[:8]}... on epoch {epoch}"
            )
...
def wait_for_event_confirmation(event_hash, timeout=120):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            ) > 10:  # Simulate confirmation after 10 seconds
                logger.info(
                    f"Event {event_hash} confirmed after {time.time() - start_time:.1f} seconds"
                )
                return True
...
def validate_contract():
    ...
    try:
        ...
        logger.info(
            f"Contract at {CONTRACT_ADDRESS} is valid with balance of {balance_bnb:.4f} BNB"
        )
        return {
            "valid": True,
