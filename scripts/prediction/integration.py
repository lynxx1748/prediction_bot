...
from ..analysis.technical import detect_market_reversal, detect_combined_technical_reversal
...
def integrate_reversal_detection(predictions, confidences, weights):
    ...
    try:
        ...
        if reversal_pred != "UNKNOWN" and reversal_conf > 0.65:
            logger.warning(f"⚠️ REVERSAL ALERT! {reversal_pred} ({reversal_conf:.2f}): {reason}")
        ...
        if combined_reversal and combined_reversal['detected'] and reversal_pred == "UNKNOWN":
            # Only add this if the main reversal detector didn't fire
            logger.warning(f"⚠️ TECHNICAL REVERSAL PATTERN: {combined_reversal['direction']} ({combined_reversal['confidence']:.2f})")
            ...
            total_other_weight = sum(v for k, v in weights.items() if k != 'tech_reversal')
    ...
    try:
        ...
        if market_data and 'bnb_24h_change' in market_data:
            ...
            if abs(change) > 5.0:  # Significant 24h move
                ...
                trend_conf = min(0.55 + abs(change) / 20, 0.75)  # Scale with move size, cap at 0.75
                
                logger.info(f"📈 Large 24h price move detected: {change:.2f}% - adding trend signal")
        ...
        if bull_weight > bear_weight:
            ...
            final_prediction = tech_pred if tech_pred else "BULL"  # Default to BULL if no tech prediction
            confidence = 0.5 + (tech_conf * 0.02)  # Use tech_conf to slightly adjust confidence
        
        # Cap confidence at 0.95
        confidence = min(confidence, 0.95)
        
        logger.info(f"🔄 Integrated {len(predictions)} signals: {final_prediction} with {confidence:.2f} confidence")
