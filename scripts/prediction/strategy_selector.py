    ...
    try:
        ...
        if sample_size < 5:
            ...
            if sample_size == 0:
                ...
                logger.info(f"Building historical data ({sample_size}/5 samples collected)")
            else:
                logger.info(f"Almost ready for strategy optimization ({sample_size}/5 samples)")
        ...
        strategies_with_data = [s for s in performance_data.keys() 
                               if performance_data[s].get('sample_size', 0) >= 5]
        
        if not strategies_with_data:
            logger.info(f"Not enough samples for {regime} regime strategies - using defaults")
            return get_fallback_strategy(market_regime)
        ...
        if top_strategies and top_strategies[0][1] > 0.52:
            ...
            logger.info(f"🔄 Selected {primary_strategy} as primary strategy (score: {top_strategies[0][1]:.2f})")
            ...
            if len(top_strategies) > 1 and top_strategies[1][1] > 0.5:
                ...
                logger.info(f"🔄 Selected {secondary_strategy} as backup strategy (score: {top_strategies[1][1]:.2f})")
        ...
        if regime == "uptrend":
            ...
            return {"primary": "mean_reversion", "secondary": "volume_analysis", "score": 0.5}
        else:  # ranging
            return {"primary": "contrarian", "secondary": "support_resistance", "score": 0.5}
            
    except Exception as e:
        logger.error(f"❌ Error selecting optimal strategy: {e}")
        traceback.print_exc()
        return {"primary": "trend_following", "secondary": None, "score": 0.5}  # Safe fallback
...
def get_fallback_strategy(market_regime):
    ...
    try:
        ...
        for strategy, prob in regime_strategies:
            cumulative += prob
            if r <= cumulative:
                logger.info(f"🔍 Using default {strategy} strategy for {regime} market (no historical data)")
...
def analyze_round_characteristics(round_data):
    ...
    if total_amount > 0:
        ...
        characteristics['is_high_volume'] = total_amount > 1.0  # Adjust threshold as needed
