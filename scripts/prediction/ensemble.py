...
from ..analysis.market import (fetch_market_data,
    ...
    try:
        ...
        if "prices" in round_data and round_data["prices"]:
            try:
                ...
                pattern_pred, pattern_conf = detect_advanced_patterns(
                    round_data["prices"]
                )
        ...
        for strategy in predictions:
            logger.info(
                f"   {strategy.upper()}: {predictions[strategy]} ({confidences[strategy]:.2f})"
            )
...
async def get_trend_prediction_async(round_data):
    ...
    try:
        ...
        logger.info(
            f"Trend prediction: {prediction} ({confidence:.2f}) based on {trend}"
        )
