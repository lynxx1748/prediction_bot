...
def analyze_prediction_accuracy(config):
    ...
    try:
        ...
        consecutive_wrong_threshold = contrarian_config.get(
        ...
        if recent_predictions:
            ...
            for pred in recent_predictions[:3]:
                outcome = (
                    "✅ Correct"
                    if pred.get("final_prediction") == pred.get("actual_outcome")
                    else "❌ Wrong"
                )
                logger.info(
                    f"  Round {pred.get('epoch')}: {pred.get('final_prediction')} vs {pred.get('actual_outcome')} - {outcome}"
                )
        ...
        if accuracy == 0.5 and consecutive_wrong == 0:
            ...
            try:
                ...
                if trend and strength > 0.7:
                    logger.info(
                        f"📈 Strong {trend.upper()} trend detected (strength: {strength:.2f})"
                    )
                    logger.info(
                        f"⚠️ Using trend data for contrarian decision - CONTRARIAN MODE ACTIVATED"
                    )
                    return True
            ...
            logger.info(
                "📝 Not enough data for contrarian analysis - using regular mode"
            )
            return False
        ...
        if accuracy < accuracy_threshold:
            logger.warning(
                f"⚠️ Low prediction accuracy ({accuracy:.2f}) - CONTRARIAN MODE ACTIVATED"
            )
            return True
        elif consecutive_wrong >= consecutive_wrong_threshold:
            logger.warning(
                f"⚠️ {consecutive_wrong} consecutive wrong predictions - CONTRARIAN MODE ACTIVATED"
            )
            return True
...
def analyze_volume(bull_amount, bear_amount):
    ...
    try:
        ...
        logger.info(
            f"📊 Volume-based prediction: {prediction} with {confidence:.2f} confidence"
        )
...
def analyze_sentiment(market_data):
    ...
    try:
        ...
        logger.info(
            f"💭 Sentiment-based prediction: {prediction} with {confidence:.2f} confidence"
        )
...
def get_consolidated_analysis(round_data, market_data):
    ...
    try:
        ...
        results["volume"] = {"prediction": volume_pred, "confidence": volume_conf}
        ...
        results["technical"] = {"prediction": tech_pred, "confidence": tech_conf}
        ...
        if signals:
            ...
            results["signals_agreement"] = (bull_count == len(signals)) or (
                bear_count == len(signals)
            )
            ...
            if bull_count > bear_count:
                ...
                results["confidence"] = avg_confidence * (
                    0.8 + (0.2 * results["signals_agreement"])
                )
...
def evaluate_strategy_performance(strategy_name, lookback=100):
    ...
    try:
        ...
        if (
            "strategies" in performance_data
            and strategy_name in performance_data["strategies"]
        ):
            ...
            logger.info(
                f"Recommended Mode: {'Contrarian' if contrarian_mode else 'Normal'}"
            )
...
def get_prediction_config_settings():
    ...
    try:
        ...
        prediction_settings = {
            ...
            "accuracy_threshold": config.get("contrarian_mode", {}).get(
                "accuracy_threshold", 0.35
            ),
            "volume_weight": config.get("prediction", {}).get("volume_weight", 0.3),
            "sentiment_weight": config.get("prediction", {}).get(
                "sentiment_weight", 0.2
            ),
            "technical_weight": config.get("prediction", {}).get(
                "technical_weight", 0.5
            ),
            "high_confidence_threshold": config.get("prediction", {}).get(
                "high_confidence", 0.75
            ),
        }

        logger.info(
            f"Loaded prediction settings from config: min_confidence={prediction_settings['min_confidence']}"
        )
