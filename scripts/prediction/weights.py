    ...
    try:
        ...
        if signal_performance:
            for signal in signal_performance:
                ...
                logger.info(
                    f"  {signal['signal']}: {accuracy:.2f} accuracy ({signal['correct']}/{signal['total']})"
                )
        else:
        ...
        if prices and len(prices) > 15:
            ...
            logger.info(
                f"Current Market Regime: {market_regime.upper()} (confidence: {regime_confidence:.2f})"
            )

        if not recent_predictions:
            logger.warning(
                "No recent predictions with outcomes found. Using market regime optimized weights."
            )
        ...
        prediction_weights = [
            time_decay_factor**i for i in range(len(recent_predictions))
        ]
        ...
        if bull_accuracy["total"] > 0:
            ...
            logger.info(
                f"  BULL predictions: {bull_acc:.2f} ({bull_accuracy['correct']:.2f}/{bull_accuracy['total']:.2f})"
            )
        if bear_accuracy["total"] > 0:
            bear_acc = bear_accuracy["correct"] / bear_accuracy["total"]
            logger.info(
                f"  BEAR predictions: {bear_acc:.2f} ({bear_accuracy['correct']:.2f}/{bear_accuracy['total']:.2f})"
            )
        ...
        if total_success == 0:
            ...
            sum_exceeding_raw = sum(
                raw_weights[s] for s in adjusted_weights if adjusted_weights[s] is None
            )
                ...
                for strategy in adjusted_weights:
                    if adjusted_weights[strategy] is None:
                        adjusted_weights[strategy] = (
                            raw_weights[strategy] / sum_exceeding_raw
                        ) * remaining_weight
...
def apply_regime_boost(weights, regime, confidence):
    ...
    if regime == "uptrend":
        ...
        if "trend_following" in result:
            result["trend_following"] = min(
                result["trend_following"] * (1 + boost_factor), 0.40
            )
    elif regime == "downtrend":
        # Moderate boost to trend following and market indicators in downtrends
        if "trend_following" in result:
            result["trend_following"] = min(
                result["trend_following"] * (1 + boost_factor * 0.5), 0.35
            )
        if "market_indicators" in result:
            result["market_indicators"] = min(
                result["market_indicators"] * (1 + boost_factor * 0.5), 0.40
            )
    elif regime == "ranging":
        ...
        if "volume_analysis" in result:
            result["volume_analysis"] = min(
                result["volume_analysis"] * (1 + boost_factor), 0.40
            )
