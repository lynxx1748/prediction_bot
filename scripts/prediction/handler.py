...
from ..core.constants import STRATEGY_WEIGHTS, MARKET_BIAS
...
class PredictionHandler:
    ...
    async def get_prediction(self, round_data, strategy_preference=None):
        ...
        try:
            ...
            for strategy_name, strategy_instance in self.strategies.items():
                if hasattr(strategy_instance, 'predict') and callable(strategy_instance.predict):
                    tasks.append(self._get_strategy_prediction(strategy_name, strategy_instance, round_data))
            ...
            for strategy_name in predictions:
                ...
                if strategy_preference and strategy_name == strategy_preference:
                    weights[strategy_name] = self.strategy_weights.get(strategy_name, 0.1) * 1.5  # 50% boost
                else:
                    weights[strategy_name] = self.strategy_weights.get(strategy_name, 0.1)
            ...
            predictions, confidences, weights = self._apply_market_bias(predictions, confidences, weights)
            ...
            logger.info(f"📊 Final prediction: {final_prediction} with {final_confidence:.2f} confidence")
            for strategy_name, prediction in predictions.items():
                logger.info(f"  - {strategy_name}: {prediction} ({confidences[strategy_name]:.2f}, weight: {weights[strategy_name]:.2f})")
    ...
    def _apply_market_bias(self, predictions, confidences, weights):
        ...
        if avg_confidence >= min_confidence:
            ...
            total_other_weight = sum(w for k, w in weights.items() if k != 'market_bias')
            ...
            logger.info(f"🧲 Applied {bias_direction} market bias (strength: {bias_strength:.2f})")
    ...
    async def _update_market_regime(self):
        ...
        try:
            ...
            if prices and len(prices) > 15:
                ...
                logger.info(f"🔍 Updated Market Regime: {self.current_regime.upper()} (confidence: {self.regime_confidence:.2f})")
    ...
    def get_all_predictions(self):
        ...
        if hasattr(self, 'trend_following_prediction') and self.trend_following_prediction:
        ...
        if hasattr(self, 'volume_analysis_prediction') and self.volume_analysis_prediction:
    ...
    def add_prediction(self, strategy_type, prediction, confidence):
        ...
        try:
            ...
            logger.info(f"✅ Added {strategy_type} prediction: {prediction} ({confidence:.2f} confidence)")
            return True
        except Exception as e:
    ...
    def get_strategy_weights(self):
        ...
        try:
            ...
            if hasattr(self, 'market_condition') and self.market_condition == 'volatile':
    ...
    def get_prediction(self, round_data, strategy_preference=None):
        ...
        try:
            ...
            if strategy_preference and strategy_preference in all_predictions:
                ...
                logger.info(f"Using {strategy_preference} strategy: {prediction} ({confidence:.2f})")
