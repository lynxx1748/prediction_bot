...
def filter_signals(predictions, min_confidence=0.55, market_regime=None):
    ...
    try:
        ...
        # Debug information
        logger.info(
            f"Filtering signals for {len(predictions)} predictions with min threshold {min_confidence}"
        )
        for name, pred in predictions.items():
            logger.info(
                f"  - {name}: {pred.get('prediction')} ({pred.get('confidence', 0):.2f})"
            )
        ...
        for signal_type, pred_data in predictions.items():
            ...
            if signal_type in strategy_success and len(strategy_success) > 5:
                ...
                if success_rate >= 0.55:  # Good performance
                    ...
                    logger.warning(
                        f"Reducing {signal_type} weight due to poor performance"
                    )
            ...
            logger.info(
                f"Signal {signal_type} ({prediction}) received quality score: {quality:.2f}"
            )
        ...
        if len(strategy_success) > 10:
            ...
            overall_success = (
                sum(strategy_success.values()) / len(strategy_success)
                if strategy_success
                else 0.5
            )
            ...
            logger.info(
                f"Using permissive threshold ({min_threshold}) to gather training data"
            )
        ...
        for signal_type, quality in quality_scores.items():
            if quality >= min_threshold:
                ...
                logger.info(
                    f"✅ {signal_type} passed filtering with quality {quality:.2f}"
                )
            else:
                logger.info(
                    f"❌ {signal_type} filtered out with quality {quality:.2f} < {min_threshold}"
                )
        ...
        if len(filtered_predictions) >= 2 and len(strategy_success) > 10:
            bull_count = sum(
                ...
                if v.get("prediction") == "BULL"
            )
            bear_count = sum(
                1
                for k, v in filtered_predictions.items()
                if v.get("prediction") == "BEAR"
            )
            ...
            if bull_count > 0 and bear_count > 0 and overall_success < 0.45:
                ...
                logger.info(
                    f"🔍 Applying stricter filtering due to mixed signals and poor performance"
                )
                ...
                for signal_type, pred_data in filtered_predictions.items():
                    if pred_data.get("quality", 0) >= stricter_threshold:
                        ...
                        logger.info(
                            f"❌ {signal_type} filtered out by stricter threshold"
                        )
        ...
        if not filtered_predictions:
            ...
            for signal_type, pred_data in predictions.items():
                ...
                if (
                    conf > highest_conf and conf >= 0.6
                ):  # Must have at least 0.6 confidence
                    highest_conf = conf
            ...
            if highest_signal:
                ...
                logger.info(
                    f"🔄 No predictions passed filters but allowing {highest_signal} with {highest_conf:.2f} confidence to collect data"
                )
