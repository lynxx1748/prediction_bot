...
from ..analysis.market import get_market_direction, get_market_sentiment
...
class Strategy(ABC):
    ...
    def __init__(self, name=None, config=None):
        ...
        self.historical_performance = {"predictions": 0, "correct": 0, "accuracy": 0.5}
    ...
    def analyze_performance_stats(self):
        ...
        try:
            ...
            if len(self.prediction_history) < 3:
                return {"confidence_mean": 0, "confidence_std": 0, "consistency": 0}
            ...
            correct_predictions = np.array(
                [1 if p.get("correct", False) else 0 for p in self.prediction_history]
            )
            ...
            stats = {
                ...
                "consistency": float(
                    np.mean(correct_predictions[-10:])
                    if len(correct_predictions) >= 10
                    else np.mean(correct_predictions)
                ),
                "recent_accuracy": float(
                    np.sum(correct_predictions[-20:])
                    / min(20, len(correct_predictions))
...
class SwingTradingStrategy(Strategy):
    ...
    def predict(self, round_data):
        ...
        try:
            from ..analysis.swing import (detect_price_swing,
...
def get_strategy(strategy_name, config=None):
    ...
    if strategy_class:
        ...
        logger.warning(
            f"⚠️ Unknown strategy: {strategy_name}, using TrendFollowingStrategy"
        )
...
def get_optimal_strategy(market_regime, historical_performance, round_data):
    ...
    strategy_selection = select_optimal_strategy(
        market_regime, historical_performance, round_data
    )
