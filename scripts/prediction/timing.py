    ...
    try:
        ...
        if stored_preferences:
            ...
            if str(current_epoch) in stored_preferences:
                ...
                logger.info(
                    f"Found stored timing preferences for epoch {current_epoch}"
                )
        ...
        if regime == "volatile":
            ...
            logger.debug(
                f"Using volatile market timing strategy (optimal_pct={optimal_pct})"
            )
            ...
            optimal_pct = 0.65
            logger.debug(
                f"Using trending market timing strategy (optimal_pct={optimal_pct})"
            )
        ...
        if is_optimal_time:
            logger.info(
                f"🕒 OPTIMAL ENTRY TIME: {time_to_lock:.1f}s before lock ({current_pct:.2f} vs optimal {optimal_pct:.2f})"
            )
        else:
            logger.debug(
                f"Not optimal entry time: {time_to_lock:.1f}s before lock ({current_pct:.2f} vs optimal {optimal_pct:.2f})"
            )
        ...
        logger.error(
            f"❌ Error calculating optimal entry time for epoch {current_epoch}: {e}"
        )
        traceback.print_exc()
        return {
            "epoch": current_epoch,
