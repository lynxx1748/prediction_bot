...
def validate_trade_signal(prediction, confidence, market_regime, round_data):
    ...
    try:
        ...
        if prediction == "BULL" and bear_ratio > 0.9:  # INCREASED from 0.75
            logger.warning(f"Rejecting BULL signal - market too bearish ({bear_ratio:.2f})")
            return False, "extreme_sentiment"
            
        if prediction == "BEAR" and bull_ratio > 0.9:  # INCREASED from 0.75
            logger.warning(f"Rejecting BEAR signal - market too bullish ({bull_ratio:.2f})")
            return False, "extreme_sentiment"
        ...
        try:
            ...
            if recent_signals and len(recent_signals) >= 3:
                recent_accuracy = sum(1 for s in recent_signals if s['prediction'] == s['outcome']) / len(recent_signals)
                
                # LOWERED from 0.4 to 0.3
                if recent_accuracy < 0.3 and confidence < 0.65:  # REDUCED from 0.7 
                    logger.warning(f"Rejecting signal - recent accuracy low ({recent_accuracy:.2f}) and confidence not high enough")
        ...
        try:
            ...
            if pattern_str > 0.6:  # INCREASED from 0.6
                if (pattern_dir == "bullish" and prediction == "BEAR") or (pattern_dir == "bearish" and prediction == "BULL"):
                    # Prediction contradicts strong pattern
                    if confidence < 0.7:  # REDUCED from 0.75
                        logger.warning(f"Rejecting {prediction} signal - contradicts strong {pattern_dir} pattern")
