...
def calculate_optimal_bet_size(wallet_balance=1.0, win_rate=0.55, strategy_confidence=0.6, market_regime=None, base_amount=None):
    """
    ...
    if base_amount is not None:
        ...
        return _calculate_simple_bet_size(base_amount, strategy_confidence, wallet_balance)
    
    # Otherwise, use the original complex logic
    return _calculate_kelly_bet_size(wallet_balance, win_rate, strategy_confidence, market_regime)
...
def _calculate_simple_bet_size(base_amount, confidence, balance):
    ...
    try:
        ...
        logger.info(f"💰 Simple bet sizing: {bet_amount:.5f} BNB (from base: {base_amount}, conf: {confidence:.2f})")
        return bet_amount
...
def _calculate_kelly_bet_size(wallet_balance, win_rate, strategy_confidence, market_regime):
    """
    ...
    try:
        ...
        if sample_size < 5:
            logger.info(f"🔍 Forcing minimal bets to build history (sample size: {sample_size})")
...
def calculate_risk_adjusted_bet_size(wallet_balance, win_rate, confidence, risk_level='medium'):
    """
    ...
    logger.info(f"💰 Risk-adjusted bet: {bet_amount:.5f} BNB ({bet_percentage*100:.1f}% of wallet)")
    return bet_amount

def get_adaptive_bet_size(wallet_balance, win_rate, confidence, market_regime, losing_streak=0):
    """
    ...
    if losing_streak > 0:
        ...
        reduction_factor = max(0.5, 1 - (losing_streak * 0.1))  # Reduce by 10% per loss, min 50%
        adjusted_bet = optimal_bet * reduction_factor
        
        logger.info(f"⚠️ On a losing streak ({losing_streak} losses) - reducing bet by {(1-reduction_factor)*100:.0f}%")
