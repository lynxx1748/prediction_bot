...
from ..data.database import (
    ...
    get_prediction_sample_size,
    get_market_balance_stats,
...
from ..utils.helpers import optimize_swing_trading, detect_swing_pattern
    ...
    try:
        ...
        if isinstance(round_data, dict) and 'epoch' in round_data:
            ...
            if current_epoch in placed_bets:
                logger.warning(f"⚠️ Already placed a bet for epoch {current_epoch}, skipping")
                return False
        ...
        try:
            try:
                ...
                logger.warning(f"Error with primary market stats: {e}, trying robust version")
        ...
        if swing_opportunity.get('swing_opportunity', False):
            ...
            if direction == prediction:
                logger.info(f"✅ OPTIMAL SWING ENTRY: {direction} with {swing_conf:.2f} confidence")
                
                # Reduced minimum pool size for high-quality swing trades
                if total_amount >= 0.05:  # Very low threshold for swing trades
                    return True
                else:
                    logger.warning(f"⚠️ Pool size too small even for swing trade: {total_amount:.3f}")
                    return False
        ...
        if confidence < min_confidence:
            logger.warning(f"⚠️ Confidence too low: {confidence:.2f} (min: {min_confidence:.2f})")
            return False
        ...
        if recent_trades and len(recent_trades) >= 3:
            ...
            if price_changes:
                ...
                if recent_volatility > 0.02:  # 2% average change is very high for 6-minute periods
                    logger.warning(f"⚠️ Extremely high volatility detected: {recent_volatility:.4f} - skipping bet")
                    return False
        ...
        if recent_predictions and len(recent_predictions) >= 2:
            ...
            if latest_pred and latest_pred != prediction:
                logger.warning(f"⚠️ Conflicting with previous prediction: {latest_pred} vs current {prediction}")
                # Require higher confidence for conflicting predictions
                if confidence < 0.49:
                    logger.warning(f"⚠️ Confidence too low for conflicting prediction: {confidence:.2f} < 0.49")
                    return False
                    
                # If we have conflicting signals, require higher total amount
                min_total = config.get('thresholds', {}).get('min_total_amount', 0.1) * 1.5
                if total_amount < min_total:
                    logger.warning(f"⚠️ Pool size too small for conflicting signals: {total_amount:.2f} < {min_total:.2f}")
                    return False
        ...
        if prediction == 'BULL' and trend == 'DOWN' and strength < 0.7:
            logger.info(f"✅ Optimal BULL entry on minor dip (trend: {trend}, strength: {strength:.2f})")
            min_confidence -= 0.05
        
        # For bear predictions, prefer entering on minor rallies
        elif prediction == 'BEAR' and trend == 'UP' and strength < 0.7:
            logger.info(f"✅ Optimal BEAR entry on minor rally (trend: {trend}, strength: {strength:.2f})")
            min_confidence -= 0.05
        
        # 5. POOL SIZE CHECK
        if total_amount < min_total:
            logger.warning(f"⚠️ Total pool amount too low: {total_amount:.2f} (min: {min_total:.2f})")
            return False
            
        # 6. WIN RATE TRACKING: If we're on a losing streak, be more selective
        performance = get_recent_performance(10)
        
        if performance and performance.get('streak', 0) >= 2 and performance.get('streak_type', '') == 'loss':
            # On a losing streak, require higher confidence
            streak_min_confidence = min_confidence + 0.05 * min(performance['streak'], 3)
            logger.warning(f"⚠️ On a {performance['streak']} loss streak - requiring higher confidence: {streak_min_confidence:.2f}")
            
            if confidence < streak_min_confidence:
                logger.warning(f"⚠️ Confidence too low during losing streak: {confidence:.2f} < {streak_min_confidence:.2f}")
                return False
        ...
        if sample_size < 5:
            ...
            if confidence >= 0.6 and total_amount >= 0.05:
                logger.info(f"🔍 Early learning phase: Placing bet with confidence {confidence:.2f}")
                return True
...
def calculate_bet_amount(wallet_balance, base_amount, strategy, confidence=0.0, history=None):
    """
    ...
    try:
        ...
        # Confidence-based strategy
            ...
            logger.info(f"💡 Confidence strategy: {confidence:.2f} confidence → {confidence_factor:.2f}x multiplier")
        ...
        # Kelly Criterion
            ...
            if kelly_fraction <= 0:
                logger.warning(f"⚠️ Kelly suggests no bet (fraction: {kelly_fraction:.2f})")
                # For early phase, override kelly's "no bet" recommendation
                if get_prediction_sample_size() < 5:
                    logger.info("💰 Overriding Kelly's 'no bet' recommendation for early learning phase")
            ...
            logger.info(f"💡 Kelly strategy: {win_prob:.2f} win rate → {conservative_kelly:.2f}x multiplier")
            return min(bet_amount, max_bet)
        ...
        # Martingale
            ...
            martingale_factor = min(2 ** consecutive_losses, 4)  # Cap at 4x to avoid excessive risk
            bet_amount = base_amount * martingale_factor
            
            if consecutive_losses > 0:
                logger.info(f"💡 Martingale strategy: {consecutive_losses} losses → {martingale_factor:.2f}x multiplier")
            
            return min(bet_amount, max_bet)
            
        else:
            logger.warning(f"⚠️ Unknown betting strategy: {strategy}, using fixed amount")
            return base_amount
...
def place_bet(web3, contract, wallet_address, private_key, prediction, confidence, round_epoch, round_data, config):
    """
    ...
    try:
        ...
        if recent_pred and recent_pred.get('prediction'):
            ...
            if recent_pred['prediction'].upper() != prediction.upper():
                ...
                min_confidence = config.get('trading', {}).get('conflict_confidence', 0.7)
                
                if confidence < min_confidence:
                    logger.warning(f"⚠️ Confidence too low for conflicting prediction: {confidence:.2f} < {min_confidence}")
                    return False
                    
                # If confidence is high enough, log the override
                logger.info(f"🔄 Overriding previous {recent_pred['prediction']} prediction with {prediction} (confidence: {confidence:.2f})")
        ...
        if wallet_balance < min_balance:
            logger.warning(f"⚠️ Wallet balance too low: {wallet_balance:.4f} BNB (min: {min_balance:.4f})")
            return False
        ...
        betting_history = get_betting_history(20) if bet_strategy in ['kelly', 'martingale'] else None
        ...
        logger.info(f"🧮 Using {bet_strategy} strategy: {base_amount:.4f} → {adjusted_bet_amount:.4f} BNB")
        
        # Determine if this is test mode
        betting_mode = config.get('trading', {}).get('betting_mode', 'test')
        
        if betting_mode.lower() == "test":
            logger.info(f"🧪 TEST MODE: Simulated {prediction} bet of {adjusted_bet_amount:.4f} BNB on round {round_epoch}")
            _log_bet(round_epoch, prediction, adjusted_bet_amount, "TEST")
            return True
            
        elif betting_mode.lower() == "live":
            return _place_live_bet(web3, contract, wallet_address, private_key, 
                                  prediction, adjusted_bet_amount, round_epoch, 
                                  config.get('trading', {}).get('gas_strategy', 'medium'))
        else:
            logger.warning(f"⚠️ Invalid betting mode: {betting_mode}. Must be 'test' or 'live'.")
            return False
...
def _log_bet(round_epoch, prediction, amount, mode):
    ...
    with open("betting_log.txt", "a") as log_file:
        log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {mode} BET: {prediction.upper()} bet of {amount} BNB on round {round_epoch}\n")

def _place_live_bet(web3, contract, wallet_address, private_key, prediction, amount, current_epoch, gas_strategy):
    """
    ...
    try:
        ...
        func = contract.functions.betBull if prediction == "bull" else contract.functions.betBear
...
def get_most_recent_prediction():
    ...
    try:
        ...
        return {
            ...
            'time_ago': int(time.time()) - latest.get('timestamp', 0) if latest.get('timestamp') else 0
        }
