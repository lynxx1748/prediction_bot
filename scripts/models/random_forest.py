...
def get_training_data():
    ...
    try:
        ...
        cursor.execute(f"SELECT COUNT(*) FROM {TABLES['predictions']} WHERE actual_outcome IS NOT NULL")
        outcome_records = cursor.fetchone()[0]
        
        logger.info(f"📊 Database contains {total_records} total records, {outcome_records} with outcomes")
        ...
        logger.info(f"✅ Got {len(X)} usable training samples from {len(rows)} database records")
        return {'X': X, 'y': y}
...
def train_model(force=False):
    ...
    try:
        ...
        if training_data is None or len(training_data['X']) < config.get("model", {}).get("update_threshold", 50):
            logger.warning(f"⚠️ Not enough training data: {len(training_data['X']) if training_data else 0} samples")
            return None, None
...
def evaluate_model_performance(test_size=0.2):
    ...
    try:
        ...
        if training_data is None or len(training_data['X']) < 50:
            logger.warning(f"⚠️ Not enough data for evaluation: {len(training_data['X']) if training_data else 0} samples")
            return None
...
def optimize_model_hyperparameters():
    ...
    try:
        ...
        if training_data is None or len(training_data['X']) < 100:
            logger.warning(f"⚠️ Not enough data for optimization: {len(training_data['X']) if training_data else 0} samples")
            return None
