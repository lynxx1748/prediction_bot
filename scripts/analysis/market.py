    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"⚠️ Regular Binance API returned {response.status_code}")
        except Exception as e:
        ...
        synthetic_prices = [base_price * (1 + random.uniform(-0.02, 0.02)) for _ in range(lookback)]
...
def get_historical_volumes(lookback=30):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.warning(f"⚠️ Error fetching historical volumes: HTTP {response.status_code}")
            return None
...
def get_market_sentiment(round_data=None):
    ...
    try:
        ...
        if round_data is None:
            logger.warning("No round data provided for market sentiment analysis, using defaults")
            return "neutral", 0.5
...
async def get_prices_from_multiple_sources():
    ...
    sources = [
        ...
        # CoinGecko
        ("https://api.coingecko.com/api/v3/simple/price?ids=binancecoin&vs_currencies=usd", "binancecoin.usd"),
        # CryptoCompare
...
def analyze_market_indicators():
    ...
    try:
        ...
        if fg_category in ["Extreme Fear", "Fear"]:
            ...
            logger.info(f"F&G category signal: {fg_category} → bullish ({strength:.2f})")
            ...
            logger.info(f"F&G category signal: {fg_category} → bearish ({strength:.2f})")
...
def bootstrap_market_data():
    ...
    try:
        ...
        logger.info(f"⚠️ Limited historical data (found {count} rounds) - bootstrapping data...")
...
def get_price_trend(prices, window=10):
    ...
    try:
        ...
        recent_price = prices[0] if isinstance(prices[0], (int, float)) else prices[0].get('price', 0)
        older_price = prices[window-1] if isinstance(prices[window-1], (int, float)) else prices[window-1].get('price', 0)
...
def get_market_time_ranges():
    ...
    return {
        ...
        'yesterday': {
            ...
            'end': datetime(now.year, now.month, now.day, 0, 0, 0) - timedelta(seconds=1)
        },
        'week': {
...
def calculate_market_statistics(prices):
    ...
    stats = {
        ...
        'skewness': float(0 if len(returns) < 3 else np.sum((returns - np.mean(returns))**3) / 
...
def get_market_sentiment_report():
    ...
    try:
        ...
        if fg_category == "Extreme Fear":
            sentiment_description = "Market in extreme fear - potential buying opportunity"
        elif fg_category == "Fear":
            ...
            sentiment_description = "Market extremely greedy - potential selling opportunity"
            
        logger.info(f"Market sentiment: {fg_category} ({fg_value}) - {sentiment_description}")
