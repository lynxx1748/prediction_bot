...
from ..data.database import get_recent_trades, get_market_balance_stats
...
def analyze_short_term_momentum(recent_data=None, lookback=10):
    ...
    try:
        ...
        recent_momentum = sum(1 for p in recent if p > 0) / len(recent) if recent else 0.5
        
        # 3. Medium-term momentum (last 7+ rounds)
        medium = price_changes[7:] if len(price_changes) >= 8 else []
        medium_momentum = sum(1 for p in medium if p > 0) / len(medium) if medium else 0.5
        ...
        if micro_trend == "UP" and micro_strength > 0.6:
            ...
            logger.info(f"📈 Strong 6-min UP microtrend: {prediction} ({confidence:.2f})")
        elif micro_trend == "DOWN" and micro_strength > 0.6:
            # Strong downward microtrend
            prediction = "BEAR"
            confidence = micro_strength
            logger.info(f"📉 Strong 6-min DOWN microtrend: {prediction} ({confidence:.2f})")
        elif pattern:
            # Use pattern-based prediction
            prediction = pattern["prediction"]
            confidence = pattern["confidence"]
            logger.info(f"🔄 6-min Pattern detected: {pattern['name']} → {prediction} ({confidence:.2f})")
        elif momentum_shift > 0.3:
            # Recent positive momentum shift
            prediction = "BULL"
            confidence = 0.6 + min(momentum_shift, 0.3)
            logger.info(f"🔼 6-min Positive momentum shift: {prediction} ({confidence:.2f})")
        elif momentum_shift < -0.3:
            # Recent negative momentum shift
            prediction = "BEAR"
            confidence = 0.6 + min(abs(momentum_shift), 0.3)
            logger.info(f"🔽 6-min Negative momentum shift: {prediction} ({confidence:.2f})")
...
def detect_price_pattern(price_changes):
    ...
    try:
        ...
        if len(sequence) >= 2 and sequence[0:2] == [-1,1] and abs(price_changes[1]) > abs(price_changes[0]):
            return {
...
def calculate_microtrend(price_changes):
    ...
    try:
        ...
        weights = np.exp(np.linspace(0, 1.5, len(price_changes)))  # Increased exponent from 1 to 1.5
        ...
        if weighted_change > 0.0008:  # Reduced from 0.001 for higher sensitivity
            ...
            strength = min(0.55 + abs(weighted_change) * 60 + consistency * 0.35, 0.95)  # Increased multipliers
...
def get_volume_acceleration(lookback=8):
    ...
    try:
        ...
        volume_changes = [volumes[i]/volumes[i+1] if volumes[i+1] > 0 else 1 
    ...
    try:
        ...
        if has_bias:
            ...
            if market_stats['bull_ratio'] > 0.6:
                ...
                logger.info(f"🔍 BOOTSTRAP BIAS: Bull trend detected ({market_stats['bull_ratio']:.2f})")
                return "BULL", bull_confidence
            elif market_stats['bear_ratio'] > 0.6:
                # Clear bear bias
                bear_confidence = 0.5 + min(market_stats['bear_ratio'] - 0.5, 0.3)
                logger.info(f"🔍 BOOTSTRAP BIAS: Bear trend detected ({market_stats['bear_ratio']:.2f})")
