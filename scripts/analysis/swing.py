...
def detect_price_swing(price_changes=None, lookback=8):
    ...
    try:
        ...
        for pc in price_changes[:5]:  # Focus on most recent 5 changes
            ...
            if prev_direction is not None and curr_direction != prev_direction and curr_direction != 0:
                direction_changes += 1
        ...
        if len(price_changes) >= 3:
            ...
            if abs(rate_change) > 0.002:  # Significant acceleration
                logger.info(f"📈 Detected {'increasing' if accel_direction > 0 else 'decreasing'} momentum")
                
                # Store for later use in decision logic
                accel_strength = min(abs(rate_change) * 40, 0.3)  # Cap at 0.3 additional confidence
            else:
                accel_strength = 0
        ...
        counter_trend = (latest_change > 0 and prev_change < 0) or (latest_change < 0 and prev_change > 0)
        ...
        if counter_trend and abs(latest_change) > abs(prev_change) * 1.5:
            ...
            if latest_change > 0:
                ...
                logger.info(f"🔄 Detected BULL SWING: Strong upward reversal ({confidence:.2f})")
            else:
                prediction = "BEAR"
                confidence = min(0.6 + abs(latest_change) * 30, 0.9)
                # Add accel_strength to confidence if acceleration aligns with prediction
                if accel_direction < 0:  # Acceleration is negative
                    confidence = min(confidence + accel_strength, 0.95)
                logger.info(f"🔄 Detected BEAR SWING: Strong downward reversal ({confidence:.2f})")
        ...
        # B. Detect momentum divergence swing
            ...
            if recent_momentum > 0:
                prediction = "BULL"
                confidence = 0.65
                logger.info(f"🔀 Momentum divergence: Shifted to BULL ({confidence:.2f})")
            else:
                prediction = "BEAR"
                confidence = 0.65
                logger.info(f"🔀 Momentum divergence: Shifted to BEAR ({confidence:.2f})")
            return prediction, confidence
        ...
        # E. Use swing market status when other signals unclear
            ...
            if latest_change > 0:
                ...
                logger.info(f"🔄 Choppy market with latest move up: Tentative BULL ({confidence:.2f})")
            elif latest_change < 0:
                prediction = "BEAR"
                confidence = 0.55  # Lower confidence in choppy markets
                logger.info(f"🔄 Choppy market with latest move down: Tentative BEAR ({confidence:.2f})")
            else:
                return "UNKNOWN", 0.0
            return prediction, confidence
...
def detect_swing_pattern(price_changes):
    ...
    try:
        ...
        if len(directions) >= 4 and directions[0:4] == [1,-1,1,-1] and \
           abs(price_changes[1]) < abs(price_changes[3]):
            return {
                "name": "Higher Lows", 
                "prediction": "BULL",
                "confidence": 0.7
        ...
        if len(directions) >= 4 and directions[0:4] == [-1,1,-1,1] and \
           price_changes[1] < price_changes[3]:
            return {
                "name": "Lower Highs",
                "prediction": "BEAR",
                "confidence": 0.7
...
def optimize_swing_trading(price_changes, threshold=0.001):
    ...
    try:
        ...
        if all(pc < 0 for pc in prev_changes) and latest_change > threshold:
            ...
            direction = "BULL"
            confidence = min(0.6 + abs(latest_change) * 30, 0.9)
            logger.info(f"⚡ Optimal BULL swing entry point detected! ({confidence:.2f})")
            ...
            direction = "BEAR"
            confidence = min(0.6 + abs(latest_change) * 30, 0.9)
            logger.info(f"⚡ Optimal BEAR swing entry point detected! ({confidence:.2f})")
            
        # Case 2: Continuation after brief counter move
        elif prev_changes[0] < 0 and prev_changes[1] > 0 and latest_change > threshold*2:
            # Bullish continuation after brief pullback
            swing_point = True
            direction = "BULL"
            confidence = 0.75
            logger.info(f"🔄 BULL continuation after pullback: Optimal entry ({confidence:.2f})")
            ...
            direction = "BEAR"
            confidence = 0.75
            logger.info(f"🔄 BEAR continuation after bounce: Optimal entry ({confidence:.2f})")
...
def calculate_swing_metrics(price_changes):
    ...
    metrics = {
        ...
        'trend_strength': float(np.abs(np.sum(changes_array)) / np.sum(np.abs(changes_array))),
