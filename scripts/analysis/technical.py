...
def calculate_microtrend(price_changes, window=5):
    ...
    try:
        ...
        recent = (
            price_changes[-window:] if len(price_changes) > window else price_changes
        )
...
def detect_market_reversal():
    ...
    """
    try:
        from ..data.processing import (get_market_trend,
        ...
        if current_trend == "BULL" and trend_duration > 5 and trend_strength > 0.7:
            ...
            if recent_avg < older_avg * 0.5:
                ...
                return (
                    "BEAR",
                    confidence,
                    "Bullish momentum weakening in extended uptrend",
                )
            ...
            if recent_avg < older_avg * 0.5:
                ...
                return (
                    "BULL",
                    confidence,
                    "Bearish momentum weakening in extended downtrend",
                )
        ...
        if len(price_changes) >= 10:
            ...
            if (
                first_half_direction != second_half_direction
                and second_half_magnitude > first_half_magnitude * 1.2
            ):
                ...
                return (
                    prediction,
                    min(confidence, 0.85),
                    f"V-shaped {prediction.lower()} reversal detected",
                )
        ...
        if len(price_points) >= 10:
            ...
            for i in range(1, len(price_points) - 1):
                ...
                if (
                    ...
                    and price_points[i] > price_points[i + 1]
                ):
            ...
            if (
                len(high_points) >= 2
                and abs(high_points[-1][1] - high_points[-2][1]) < 0.01
            ):
            ...
            if (
                len(low_points) >= 2
                and abs(low_points[-1][1] - low_points[-2][1]) < 0.01
            ):
            ...
            if len(normalized) >= 20:
                ...
                for i in range(2, len(normalized) - 2):
                    if (
                        ...
                        and normalized[i] > normalized[i - 2]
...
def analyze_market_range(prices, lookback=20):
    ...
    try:
        ...
        if position <= 0.15:
            ...
            logger.info(
                f"🔍 Price near bottom of range ({position:.2f}) - bullish signal with {confidence:.2f} confidence"
            )
            return {
                "in_range": True,
                "position": position,
            ...
            logger.info(
                f"🔍 Price near top of range ({position:.2f}) - bearish signal with {confidence:.2f} confidence"
            )
            return {
                "in_range": True,
                "position": position,
            ...
            logger.debug(
                f"Price in middle of range ({position:.2f}) - no strong signal"
            )
...
def get_ranging_market_prediction(round_data, lookback=10):
    ...
    try:
        ...
        logger.debug(
            f"Mean reversion analysis: Mean={mean_price:.2f}, Current={current_price:.2f}, Z-score={z_score:.2f}"
        )
        ...
        if z_score > 1.2:  # Price is significantly above mean
            ...
            logger.info(
                f"🔍 Price significantly above mean (z-score: {z_score:.2f}) - expecting reversion DOWN"
            )
            ...
            confidence = min(0.5 + abs(z_score) * 0.1, 0.85)
            logger.info(
                f"🔍 Price significantly below mean (z-score: {z_score:.2f}) - expecting reversion UP"
            )
            return prediction, confidence

        # If price is close to mean, no strong signal
        logger.debug(
            f"Price close to mean (z-score: {z_score:.2f}) - no strong mean reversion signal"
        )
        return None, 0
...
def get_technical_analysis(prices, config=None):
    ...
    try:
        ...
        # Weight the signals
        weights = {"rsi": 0.3, "macd": 0.3, "bb": 0.2, "ema": 0.2}
...
def get_technical_indicators_with_fallback(prices, fallback=True):
    ...
    try:
        ...
        logger.warning(
            f"Not enough price data for technical analysis (found {len(prices)})"
        )
        ...
        if prices:
            ...
            logger.info(
                f"Using combined real/synthetic price data (added {len(synthetic_prices) - len(prices)} synthetic points)"
            )
    ...
    try:
        ...
        for day in range(days_back):
            ...
            price_changes = get_recent_price_changes(
                lookback_days, end_date=signal_time
            )
