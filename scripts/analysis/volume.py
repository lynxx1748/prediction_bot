...
def analyze_volume_profile(round_data, historical_rounds):
    ...
    try:
        ...
        avg_volume = sum(r['totalAmount'] for r in historical_rounds) / len(historical_rounds)
        ...
        divergence_signal, divergence_strength = detect_volume_divergence(historical_rounds)
        ...
        if divergence_signal:
            ...
            logger.info(f"📊 Volume divergence: {divergence_signal} ({divergence_strength:.2f})")
        ...
        if volume_trend:
            if bull_ratio > bear_ratio:
                ...
                logger.info("📈 Accumulation pattern detected (rising volume with bull bias)")
            else:
                signals.append(("BEAR", 0.7))
                logger.info("📉 Distribution pattern detected (rising volume with bear bias)")
        ...
        if signals:
            ...
            logger.info(f"📊 Strongest volume signal: {strongest[0]} ({strongest[1]:.2f})")
            return strongest
    ...
    try:
        ...
        if isinstance(lookback, dict):
            ...
            try:
                ...
                logger.warning("Couldn't convert lookback parameter to integer, using default value")
                lookback = 20
        ...
        if 'totalAmount' not in columns:
            try:
                cursor.execute("ALTER TABLE '%s' ADD COLUMN totalAmount REAL DEFAULT 0" % table_name)
                cursor.execute("ALTER TABLE '%s' ADD COLUMN bullAmount REAL DEFAULT 0" % table_name)
                cursor.execute("ALTER TABLE '%s' ADD COLUMN bearAmount REAL DEFAULT 0" % table_name)
        ...
        outcome_index = columns_to_select.index('outcome') if 'outcome' in columns_to_select else 1
...
def get_volume_trend_prediction(round_data):
    ...
    try:
        ...
        if volume_increasing and price_increasing:
            ...
            prediction = "BULL"
            confidence = 0.65
            logger.info("🔄 Volume increasing but price decreasing - potential bullish reversal")
            ...
            prediction = "BEAR"
            confidence = 0.65
            logger.info("🔄 Volume decreasing but price increasing - potential bearish reversal")
...
def get_cached_volume_data(lookback=20):
    ...
    try:
        ...
        return {
            'data': [{'epoch': r[0], 'total': r[1], 'bull': r[2], 'bear': r[3]} for r in results],
...
def analyze_recent_trade_volume(lookback=10):
    ...
    try:
        ...
        increasing = all(volumes[i] >= volumes[i+1] for i in range(min(3, len(volumes)-1)))
        decreasing = all(volumes[i] <= volumes[i+1] for i in range(min(3, len(volumes)-1)))
