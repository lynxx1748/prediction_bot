...
from scripts.core.constants import (CONTRACT_ADDRESS, DB_FILE, MARKET_API,
                                    TABLES, contract, web3)
...
def fetch_historical_data(days=30, save_to_db=True):
    ...
    try:
            ...
            for epoch in range(current_epoch - max_rounds, current_epoch):
                try:
                    ...
                    if formatted_round["totalAmount"] > 0:
                        formatted_round["bullRatio"] = (
                            formatted_round["bullAmount"]
                            / formatted_round["totalAmount"]
                        )
                        formatted_round["bearRatio"] = (
                            formatted_round["bearAmount"]
                            / formatted_round["totalAmount"]
                        )
        ...
        logger.info(
            f"Successfully fetched historical data since {datetime.fromtimestamp(start_time)}"
        )
    ...
    try:
        ...
        if MARKET_API:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Failed to fetch current price: {response.status_code}"
                    )
            except Exception as e:
        ...
        if web3 and web3.is_connected() and contract:
            try:
                ...
                for epoch in range(current_epoch - 5, current_epoch + 1):
                    try:
                        ...
                        if now < formatted_round["lockTimestamp"]:
                            ...
                            if (
                                formatted_round["closePrice"]
                                > formatted_round["lockPrice"]
                            ):
                                formatted_round["outcome"] = "BULL"
                            elif (
                                formatted_round["closePrice"]
                ...
                logger.info(
                    f"Collected data for {len(real_time_data['latest_rounds'])} latest rounds"
                )
...
def ensure_data_directories():
    ...
    """
    data_dirs = ["data", "data/raw", "data/processed", "data/models", "data/logs"]
    ...
    return {"created": created_dirs, "all_dirs": data_dirs, "script_dir": script_dir}
...
def get_contract_info():
    ...
    try:
        ...
        return {"status": "error", "message": str(e), "address": CONTRACT_ADDRESS}
...
if __name__ == "__main__":
    ...
    print(
        f"Fetched {len(data['prices'])} price points and {len(data['rounds'])} rounds"
    )
