...
def get_market_trend(lookback=10):
    ...
    try:
        ...
        if up_count > down_count:
            trend = "up"
            strength = min((up_count / len(changes)) * avg_change * 0.5, 0.95) if avg_change > 0 else 0.5
        elif down_count > up_count:
            trend = "down"
            strength = min((down_count / len(changes)) * abs(avg_change) * 0.5, 0.95) if avg_change < 0 else 0.5
        else:
            trend = "neutral"
            strength = 0.5
...
def get_winning_streak():
    ...
    try:
        ...
        # Calculate streak
        streak = 0
        is_winning = results[0][0] == results[0][1]  # Win or loss for most recent prediction
...
def get_market_prediction_performance(lookback=50):
    ...
    try:
        ...
        if isinstance(lookback, dict):
            logger.warning("⚠️ Warning: lookback parameter is a dictionary, using default value")
            lookback = 50
        ...
        if not prediction_column:
            logger.warning("⚠️ No suitable column found for market prediction performance calculation")
            return {
                'accuracy': 0.5,
                'sample_size': 0,
                'total': 0,
...
def get_market_balance_stats(lookback=100):
    ...
    try:
        ...
        if total > 0:
            ...
            logger.info(f"🔍 Market Balance: {bull_ratio:.2f} BULL / {bear_ratio:.2f} BEAR over {total} rounds")
            
            # Print alert if balance is skewed
            if bull_ratio > 0.65:
                logger.warning(f"⚠️ STRONG BULL MARKET DETECTED: {bull_ratio:.2f} of recent outcomes are BULL")
            elif bear_ratio > 0.65:
                logger.warning(f"⚠️ STRONG BEAR MARKET DETECTED: {bear_ratio:.2f} of recent outcomes are BEAR")
...
def get_recent_performance(days=3, min_samples=5):
    ...
    try:
        ...
        if not recent_history or len(recent_history) < min_samples:
            logger.warning(f"Insufficient data for recent performance analysis ({len(recent_history) if recent_history else 0} samples)")
            return {
                'win_rate': 0.0,
                'bull_win_rate': 0.0,
                'bear_win_rate': 0.0,
        ...
        bull_bets = [pred for pred in recent_history if pred.get('final_prediction') == 'BULL']
        bull_wins = sum(1 for pred in bull_bets if pred.get('win') == 1)
        
        bear_bets = [pred for pred in recent_history if pred.get('final_prediction') == 'BEAR']
        ...
        return {
            ...
            'recent_outcomes': [pred.get('actual_outcome') for pred in recent_history[:10]]
        }
...
def get_overall_performance():
    ...
    try:
        ...
        for strategy, metrics in strategy_perf.items():
            if metrics.get('win_rate', 0) > best_win_rate and metrics.get('sample_size', 0) >= 10:
...
def get_recent_trades(limit=20):
    ...
    try:
        ...
        for row in rows:
            ...
            for key in ['lockPrice', 'closePrice', 'amount', 'profit_loss', 'bullRatio', 'bearRatio', 'totalAmount']:
