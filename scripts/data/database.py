        ...
        if not table_exists:
            ...
            for col_name, col_type in required_columns:
                if col_name not in existing_columns:
                    try:
                        logger.info(
                            f"Adding missing column {col_name} to predictions table"
                        )
                        cursor.execute(
                            f"ALTER TABLE {TABLES['predictions']} ADD COLUMN {col_name} {col_type}"
                        )
...
def record_prediction(epoch, data):
    ...
    try:
        ...
        cursor.execute(
            f"SELECT * FROM {TABLES['predictions']} WHERE epoch = ?", (epoch,)
        )
        ...
        if "timestamp" in existing_columns and "timestamp" not in filtered_data:
            if "time" in data:
                ...
                logger.info(
                    f"Adding current timestamp {filtered_data['timestamp']} as fallback"
                )
        ...
        if existing:
            ...
            columns = ["epoch"] + [
                key for key in filtered_data.keys() if key != "epoch"
            ]
        ...
        # Update cache
        prediction_cache.append({"epoch": epoch, **filtered_data})
...
def record_prediction_outcome(epoch, outcome):
    ...
    try:
        ...
        cursor.execute(
            f"SELECT final_prediction FROM {TABLES['predictions']} WHERE epoch = ?",
            (epoch,),
        )
        row = cursor.fetchone()
...
def update_prediction_outcome(
    epoch, outcome=None, win=None, profit_loss=None, bet_amount=None
):
    """
...
def get_overall_performance(lookback=100):
    ...
    try:
        ...
        for i in range(1, len(results)):
            if (results[i][0] == 1 and streak_type == "win") or (
                results[i][0] == 0 and streak_type == "loss"
            ):
                current_streak += 1
            else:
                break
...
def record_market_data(data):
    ...
    try:
        ...
        if "datetime" not in data and "timestamp" in data:
            data["datetime"] = datetime.fromtimestamp(data["timestamp"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
...
def initialize_database(force=False):
    ...
    try:
        ...
        for strategy_name in [
            ...
            try:
                cursor.execute(
                    f"ALTER TABLE {TABLES['predictions']} ADD COLUMN {strategy_name}_prediction TEXT"
                )
                cursor.execute(
                    f"ALTER TABLE {TABLES['predictions']} ADD COLUMN {strategy_name}_confidence REAL"
                )
    ...
    try:
        ...
        for trade in trades:
            ...
            if trade.get("lockPrice") and trade.get("closePrice"):
                price_change = (
                    (trade["closePrice"] - trade["lockPrice"]) / trade["lockPrice"]
                ) * 100
    ...
    try:
        ...
        if result:
            ...
            if stats["total_rounds"] > 0:
                stats["bull_dominant_ratio"] = (
                    stats["bull_dominant_count"] / stats["total_rounds"]
                )
                stats["bear_dominant_ratio"] = (
                    stats["bear_dominant_count"] / stats["total_rounds"]
                )
...
def get_market_balance_stats_robust(lookback=10):
    ...
    try:
        ...
        if "bullAmount" in columns and "bearAmount" in columns:
            ...
            avg_bull_ratio = (
                sum(bull_ratios) / len(bull_ratios) if bull_ratios else bull_ratio
            )
            avg_bear_ratio = (
                sum(bear_ratios) / len(bear_ratios) if bear_ratios else bear_ratio
            )
            ...
            return {
                ...
                "avg_total_amount": (total_bull + total_bear) / len(results)
                if results
                else 0,
                "max_total_amount": max(
                    [(row[0] or 0) + (row[1] or 0) for row in results]
                )
                if results
                else 0,
                "min_total_amount": min(
                    [(row[0] or 0) + (row[1] or 0) for row in results]
                )
                if results
                else 0,
                "total_rounds": len(results),
                "bull_dominant_ratio": sum(1 for r in bull_ratios if r > 0.5)
                / len(bull_ratios)
                if bull_ratios
                else 0.5,
                "bear_dominant_ratio": sum(1 for r in bear_ratios if r > 0.5)
                / len(bear_ratios)
                if bear_ratios
                else 0.5,
    ...
    try:
        ...
        if result:
            ...
            if stats["avg_bear_volume"] > 0:
                stats["bull_bear_ratio"] = (
                    stats["avg_bull_volume"] / stats["avg_bear_volume"]
                )
            else:
...
def record_trade(epoch, trade_data):
    ...
    try:
        ...
        for field in ["outcome", "prediction"]:
            if field in trade_data:
                trade_data_with_epoch[field] = (
                    str(trade_data[field]) if trade_data[field] else None
                )
