...
from ..analysis.market import get_historical_prices, get_bnb_price, get_btc_price
...
def get_round_data(epoch):
    ...
    try:
        ...
        if (lockPrice == 0 or closePrice == 0) and epoch < get_current_epoch() - 1:
            try:
                ...
                if lockPrice == 0 and current_price:
                    logger.warning(f"⚠️ Using current price {current_price} as lock price")
                    lockPrice = current_price
                if closePrice == 0 and current_price:
                    logger.warning(f"⚠️ Using current price {current_price} as close price")
        ...
        if outcome:
            logger.info(f"Outcome: {outcome} (Lock: {lockPrice:.2f} → Close: {closePrice:.2f})")
...
def get_betting_epoch():
    ...
    try:
        ...
        if betting_epoch != current_epoch:
            logger.warning(f"⚠️ Betting epoch calculation mismatch. Expected {current_epoch}, got {betting_epoch}")
    ...
    try:
        ...
        if historical_data:
            ...
            if len(historical_data) >= 3:
                ...
                prices = [data.get('closePrice', 0) for data in historical_data if data.get('closePrice', 0) > 0]
                if len(prices) >= 3:
                    round_data['price_trend'] = 'up' if prices[0] > prices[2] else 'down'
                    round_data['price_volatility'] = abs(prices[0] - prices[-1]) / prices[-1] if prices[-1] > 0 else 0
    ...
    try:
        ...
        if bnb_price:
            ...
            if round_data.get('lockPrice', 0) > 0:
                round_data['bnb_price_diff'] = (bnb_price - round_data['lockPrice']) / round_data['lockPrice'] * 100
        ...
        if bnb_price and btc_price:
            logger.info(f"📊 Current market prices - BNB: ${bnb_price:.2f}, BTC: ${btc_price:.2f}")
...
def get_round_info(config):
    ...
    try:
        ...
        timing_config = config.get("timing", {}).get("optimal_betting_seconds_before_lock", {})
        ...
        betting_window_open = (current_timestamp > current_startTimestamp and current_timestamp < current_lockTimestamp)
        ...
        if betting_window_open:
            if optimal_betting_time:
                logger.info(f"🎯 OPTIMAL BETTING WINDOW: {minutes_until_lock}m {seconds_until_lock_remainder}s until lock")
            else:
                logger.info(f"🕒 Betting window open: {minutes_until_lock}m {seconds_until_lock_remainder}s until lock")
        else:
            logger.info(f"⏳ Betting window closed. Next round in {minutes_until_close}m {seconds_until_close_remainder}s")
...
def analyze_price_history(lookback_days=30, interval='1d'):
    ...
    try:
        ...
        ma7 = sum(prices[-7:]) / min(7, len(prices)) if len(prices) >= 7 else current_price
        
        logger.info(f"Price analysis: Current: {current_price:.2f}, Change: {price_change:.2%}")
