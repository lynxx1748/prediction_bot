...
class DataCollector:
    ...
    def get_market_data(self):
        ...
        try:
            ...
            try:
                bnb_response = requests.get(
                    f"{self.binance_api}/ticker/24hr?symbol=BNBUSDT"
                )
                if bnb_response.status_code == 200:
                    ...
                    data.update(
                        {
                            "bnb_price": float(
                                bnb_data.get("lastPrice", bnb_data.get("price", 0))
                            ),
                            "bnb_24h_volume": float(bnb_data.get("volume", 0)),
                            "bnb_24h_change": float(
                                bnb_data.get("priceChangePercent", 0)
                            ),
                    ...
                    logger.error(
                        f"❌ Error fetching BNB data: HTTP {bnb_response.status_code}"
                    )
                    # Set default values
                    data.update(
                        {
            ...
            try:
                ...
                if btc_response.status_code == 200:
                    ...
                    data.update(
                        {
                            "btc_price": float(
                                btc_data.get("lastPrice", btc_data.get("price", 0))
                            ),
                            "btc_24h_volume": float(btc_data.get("volume", 0)),
                            "btc_24h_change": float(
                                btc_data.get("priceChangePercent", 0)
                            ),
                        }
                    )
                else:
                    logger.error(
                        f"❌ Error fetching BTC data: HTTP {btc_response.status_code}"
                    )
                    data.update(
                        {"btc_price": 0, "btc_24h_volume": 0, "btc_24h_change": 0}
                    )
            except Exception as e:
                logger.error(f"❌ Error processing BTC data: {e}")
                data.update({"btc_price": 0, "btc_24h_volume": 0, "btc_24h_change": 0})
    ...
    def record_data_point(self, market_data, round_data):
        ...
        try:
            ...
            bullRatio = (
                round_data["bullAmount"] / totalAmount if totalAmount > 0 else 0.5
            )
            bearRatio = (
                round_data["bearAmount"] / totalAmount if totalAmount > 0 else 0.5
            )
            ...
            logger.info(
                f"   Fear & Greed: {data['fear_greed_value']} ({data['fear_greed_class']})"
            )
