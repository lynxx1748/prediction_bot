...
config_paths = ["configuration/config.json"]
...
SCALER_FILE = config.get("model", {}).get(
    ...
    try:
        if os.path.exists(abi_path):
            try:
                ...
                logger.error(f"Invalid JSON in ABI file: {abi_path}")
                continue
        else:
...
try:
    ...
    try:
        ...
        try:
            ...
            print(
                "RPC connection works but contract calls fail - likely ABI mismatch or wrong contract address"
            )
            contract = None
...
STRATEGY_WEIGHTS = config.get("trading", {}).get(
    "prediction_weights",
    {
        "model": 0.15,
        "pattern": 0.20,
...
try:
    ...
    ACCOUNT_ADDRESS = web3.to_checksum_address(
        wallet_config.get("address", "0x0000000000000000000000000000000000000000")
    )
    ...
    if ACCOUNT_ADDRESS != "0x0000000000000000000000000000000000000000":
        ...
        print(
            f"✅ Wallet connected - Balance: {web3.from_wei(balance, 'ether'):.4f} BNB"
        )
    else:
