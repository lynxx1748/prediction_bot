...
from .constants import contract, DB_FILE, TABLES
...
def get_price_trend(lookback=5):
    ...
    try:
        ...
        # Insufficient data
        if len(prices) < 3:
            logger.debug(f"Insufficient data for price trend (found {len(prices)} points)")
            return "neutral", 0.5
        ...
        if price_change > 1.0:
            ...
            logger.info(f"📈 Upward price trend detected: {price_change:.2f}% (confidence: {confidence:.2f})")
            return "up", confidence
        elif price_change < -1.0:
            confidence = min(0.5 + abs(price_change)/10, 0.9)
            logger.info(f"📉 Downward price trend detected: {price_change:.2f}% (confidence: {confidence:.2f})")
...
def get_recent_outcomes(count=8):
    ...
    try:
        ...
        cursor.execute(f"SELECT epoch, outcome FROM {TABLES['trades']} ORDER BY epoch DESC LIMIT {count}")
        ...
        logger.info(f"Recent outcomes (last {len(outcomes)} rounds): {', '.join(outcome.upper() for outcome in outcomes)}")
        return outcomes
...
def sleep_and_check_for_interruption(seconds):
    ...
    """
    try:
        interval = 0.1  # Check every 0.1 seconds for more responsive keyboard interrupts
