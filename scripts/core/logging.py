...
def log_prediction_details(epoch, predictions, final_prediction, outcome=None):
    ...
    try:
        ...
        for strategy, pred_data in predictions.items():
            if (
                strategy != "final"
            ):  # Skip the final prediction from the predictions dict
        ...
        if outcome:
            ...
            log_entry["won"] = (
                (final_prediction.upper() == outcome.upper()) if outcome else None
            )
        ...
        for strategy, data in log_entry["strategy_predictions"].items():
            logger.info(
                f"  {strategy}: {data['prediction']} ({data['confidence']:.2f} confidence)"
            )
